"""MCP server configuration for Google Ads API."""
...
mcp_server = FastMCP("google-ads")


def login_cli():
    logger.info("Starting Google Ads CLI authentication flow")
    print("Starting Google Ads CLI authentication flow...")
    login_auth()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Google Ads MCP Server - Model Context Protocol server for Google Ads API"
    )
    ...
    if args.version:
        ...
        print(f"Google Ads MCP v{__version__}")
        return 0
    ...
    if os.environ.get("PIPEBOARD_API_TOKEN"):
        logger.warning("PIPEBOARD_API_TOKEN is set but is ignored by google-ads-mcp.")
        print(
            "⚠️  PIPEBOARD_API_TOKEN is ignored. Set GOOGLE_ADS_REFRESH_TOKEN and related env vars.",
            file=sys.stderr,
        )

    # Register tools
    from . import accounts, campaigns, ad_groups, ads, keywords, insights, authentication  # noqa: F401

    if args.transport == "streamable-http":
        logger.info(f"Starting Google Ads MCP with Streamable HTTP on {args.host}:{args.port}")
        ...
        print(f"Starting Google Ads MCP server on {args.host}:{args.port}")
        print("Auth: Authorization: Bearer <google-ads-refresh-token>")
        try:
        ...
        logger.info("Starting Google Ads MCP server with stdio transport")
