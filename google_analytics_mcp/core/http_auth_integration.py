"""FastMCP HTTP authentication for Google Analytics MCP."""
...
_auth_token: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "google_analytics_auth_token", default=None
)
...
class FastMCPAuthIntegration:
    ...
    def extract_token_from_headers(headers: dict) -> Optional[str]:
        ...
        token = headers.get("X-GOOGLE-ANALYTICS-REFRESH-TOKEN") or headers.get(
            "x-google-analytics-refresh-token"
        )
        if token:
            return token
        return None
                ...
                content=json.dumps(
                    {
                        ...
                        "message": (
                            ...
                            "<google-analytics-refresh-token> or ?token=."
                        ),
                    }
                ),
                status_code=401,
