"""Localhost OAuth callback server for Google Analytics."""
...
class CallbackHandler(BaseHTTPRequestHandler):
    def log_message(self, format, *args):
        logger.debug("Google OAuth callback: " + format % args)
    ...
    def do_GET(self):
        ...
        if error:
            ...
            html = "<html><body><h1>Google Analytics connected</h1><p>You can close this window.</p></body></html>"
        else:
            html = "<html><body><h1>Missing code</h1></body></html>"
...
def start_callback_server(preferred_port: int = 8080) -> int:
    ...
    with _server_lock:
        ...
        for candidate in range(preferred_port, preferred_port + 20):
            ...
            raise RuntimeError(f"Could not bind Google OAuth callback server: {last_error}")
        _server_port = port
        ...
        logger.info(f"Google OAuth callback server listening on 127.0.0.1:{port}")
        return port
...
def wait_for_code(timeout: int = 300) -> Optional[str]:
    ...
    while time.time() < deadline:
        ...
        if _code_container.get("error"):
            logger.error(f"Google OAuth error: {_code_container['error']}")
            return None
        time.sleep(0.5)
    return None
