"""FastMCP HTTP authentication for TikTok Ads MCP."""
...
_auth_token: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "tiktok_ads_auth_token", default=None
)
...
class FastMCPAuthIntegration:
    ...
    def extract_token_from_headers(headers: dict) -> Optional[str]:
        ...
        token = headers.get("X-TIKTOK-ACCESS-TOKEN") or headers.get("x-tiktok-access-token")
        if token:
            return token
        return None
